...
async def answer_check(answer, message):
    ...
    try:
        ...
        filtered_answer = re.search(r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?", filtered_answer)  # searching for a real number (including exp. form)
        filtered_answer = float(filtered_answer.group(0))
        if filtered_answer > 0:
          # for a clean display
            return round(filtered_answer, 1) if filtered_answer % 1 > 0 else int(filtered_answer)
        else:
